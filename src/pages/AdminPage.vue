        ...
        <el-card class="schedule-card">
          <el-form @submit.prevent="handleScheduleSave" class="schedule-form">
            <div class="schedule-form-content">
              <div v-for="(staff, index) in staffData" :key="staff.id" class="schedule-item">
                <el-input v-model="scheduleForm[index].staff" type="hidden" />
                <el-input v-model="scheduleForm[index].department" type="hidden" />
                <el-input v-model="scheduleForm[index].position" type="hidden" />
                <el-input v-model="scheduleForm[index].avatar" type="hidden" />
                <div class="schedule-item-content">
                  <el-text strong>{{ staff.name }} - {{ staff.department }} - {{ staff.position }}</el-text>
                  <el-form-item 
                    label="值班时间"
                    :prop="`scheduleForm.${index}.dutyTime`"
                    :rules="[{ required: true, message: '请选择值班时间' }]"
                  >
                    <el-select v-model="scheduleForm[index].dutyTime" placeholder="请选择值班时间" style="width: 200px">
                      <el-option label="上午 8:00-12:00" value="上午 8:00-12:00" />
                      <el-option label="下午 14:00-18:00" value="下午 14:00-18:00" />
                      <el-option label="全天 8:00-18:00" value="全天 8:00-18:00" />
                    </el-select>
                  </el-form-item>
                </div>
              </div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="照片URL" prop="avatar">
          <el-input v-model="staffForm.avatar" placeholder="请输入照片URL（可选）" />
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="医院简介" prop="introduction">
          <el-input v-model="hospitalForm.introduction" type="textarea" :rows="8" placeholder="请输入医院简介" />
        </el-form-item>
        <el-form-item label="医院科室" prop="departments">
          <el-select v-model="hospitalForm.departments" multiple placeholder="请输入科室名称，按回车确认">
            <el-option v-for="dept in departments" :key="dept" :label="dept" :value="dept" />
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="handleHospitalInfoChange">保存</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
export default {
  ...
  setup() {
    ...
    const hospitalRules = {
      ...
      introduction: [{ required: true, message: '请输入医院简介', trigger: 'blur' }],
      departments: [{ required: true, message: '请输入医院科室', trigger: 'change' }]
    }
      ...
      reader.onload = (e) => {
        try {
          ...
          const processedData = jsonData.map((item, index) => ({
            ...
            department: item['科室'] || '',
            position: item['职位'] || '',
            avatar: item['照片'] || `https://api.dicebear.com/7.x/avataaars/svg?seed=${item['姓名'] || index}`
          }))
          
          // 保存到localStorage
          localStorage.setItem('staffData', JSON.stringify(processedData))
          staffData.value = processedData
          
          // 更新科室列表
          const deptSet = new Set(departments.value)
          processedData.forEach(item => deptSet.add(item.department))
          departments.value = Array.from(deptSet)
          
          // 初始化排班表单
          scheduleForm.value = processedData.map(item => ({
            staff: item.name,
            department: item.department,
            position: item.position,
            avatar: item.avatar,
          ...
          ElMessage.success(`成功导入 ${processedData.length} 条人员信息`)
        } catch (error) {
    ...
    const handleStaffSave = async () => {
      try {
        ...
        if (editingStaff.value) {
          ...
          // 添加新人员
          const newId = staffData.value.length > 0 ? Math.max(...staffData.value.map(item => item.id)) + 1 : 1
    ...
    const handleStaffDelete = (id) => {
      ElMessageBox.confirm('确定要删除这条人员信息吗？', '确认删除', {
        ...
        // 更新排班表单
        const index = scheduleForm.value.findIndex(item => item.staff === staffData.value.find(s => s.id === id)?.name)
        if (index > -1) {
    ...
    return {
      ...
      handleHospitalModal,
      handleHospitalInfoChange,
      handleStaffModalClose,
      handlePasswordModalClose,
      handleHospitalModalClose
    }
  }
}
</script>
