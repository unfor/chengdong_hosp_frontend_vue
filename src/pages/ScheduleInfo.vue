    ...
    <div v-if="scheduleData.length > 0">
      <div v-for="dept in departments" :key="dept">
        <el-card :title="dept" class="schedule-card">
          <el-row :gutter="[16, 16]">
            <el-col 
              v-for="item in scheduleData.filter(item => item.department === dept)" 
              :key="item.id" 
              :xs="24" 
              :sm="12" 
              :md="8" 
              ...
              <div class="doctor-card">
                <div class="doctor-info">
                  ...
                  <div class="doctor-details">
                    <el-text strong class="doctor-name">{{ item.name }}</el-text>
                    <el-text class="doctor-position">{{ item.position }}</el-text>
                    <el-text class="doctor-duty-time">{{ item.dutyTime }}</el-text>
                  </div>
                </div>
              </div>
            </el-col>
